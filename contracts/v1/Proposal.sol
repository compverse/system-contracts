...
contract Proposal is Params, IProposal {
    ...
    // record
    mapping(address => bool) public override pass;
    ...
    modifier onlyValidator() {
        require(validatorsContract.isActiveValidator(msg.sender), "Validator only");
        _;
    }
    ...
    returns (bool)
    {
        require(!pass[proposals[id].dst], "Dst already passed, You can start staking");
        require(!proposals[id].resultExist, "Proposal has results");
        ...
        if (
            proposals[id].agree >=
            validatorsContract.getActiveValidators().length / 2 + 1
        ) {
            pass[proposals[id].dst] = true;
            proposals[id].resultExist = true;
            proposals[id].result = Result.SUCCESS;
        ...
        if (
            ...
            proposals[id].resultExist = true;
            proposals[id].result = Result.FAIL;
